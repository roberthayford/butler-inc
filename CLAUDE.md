...
- **Content system:** Static defaults in `src/data/` merged with Supabase-stored overrides via `src/lib/content.ts`. **`site_content` rows fully override `hero.headline` / `hero.subheading` / `trustIndicators` / `commonRequests` when present, and all 6 butlers currently have rows** — so `butler-page-configs.ts` is only the fallback for those four fields. Editing the static file alone is a no-op on the live site for any butler with a row; you must update the DB row too. Task labels (`butler-tasks.ts`), `hero.useCases`, `howItWorks`, `seo`, and `accentColor` are NOT overridable and always come from code. See Gotchas.
...
- **Butler `hero`/`trustIndicators`/`commonRequests` come from the `site_content` DB rows, NOT `butler-page-configs.ts`, on the live site.** All 6 butlers have rows; `mergeContent()` fully overrides those four fields when a row exists. Editing the static file alone changes nothing live. `butler-content.test.ts` only guards the static fallback, so a green test does not mean the live copy is correct. To change this copy you must update the DB (migration / re-seed / admin editor) and keep the static file in sync. Bug-log 2026-06-07.
