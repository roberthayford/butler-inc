...
|-----------|----------------|
| **Understated Luxury** | Muted palette, serif/sans pairing, minimal decoration |
...
|---------|-------|----------|-----|---------|
| Success/Trust | `sage` | `text-sage`, `bg-sage-light` | `#5B9473` | Body-cam badges, trust icons |
...
- **Always pair with additional context** (e.g., icons alongside text)
...
```tsx
<button className="w-full p-4 text-left rounded-lg border border-border hover:border-brass/50 hover:bg-accent/50 transition-all">
...
<div className="inline-flex rounded-xl hero-segmented-control p-1.5 gap-1">
  <Button
    className={activeSegment === "members" 
      ? "bg-optical-white text-charcoal" 
      : "text-optical-white/80 hover:bg-optical-white/10"
    }
  >
    Members
  </Button>
...
```tsx
<DialogContent className="sm:max-w-[520px] p-6 rounded-2xl bg-ivory border-0 shadow-2xl">
  <DialogHeader>
...
| Membership tiers | `src/data/membership-tiers.ts` |
